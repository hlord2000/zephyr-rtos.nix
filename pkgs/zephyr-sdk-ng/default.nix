{ stdenv, lib, hostPlatform, fetchurl, autoPatchelfHook, makeBinaryWrapper, xz, python313, which
, hidapi, libftdi1, libusb1, overrideOpenocd, cmake, wget, file
, version ? "0.17.2", toolchains ? "all" }:
...
in stdenv.mkDerivation {
  ...
  buildInputs = [ xz python313 hidapi libftdi1 libusb1 ];
  ...
  postFixup = ''
    ...
    find $out -type f -perm -a+x -name '*-py' | while read prog; do
      ...
      wrapProgram $prog \
        --set NIX_PYTHONPREFIX ${python313} \
        --set NIX_PYTHONEXECUTABLE ${python313}/bin/python3.13 \
        --set NIX_PYTHONPATH ${python313}/lib/python3.13/site-packages
    done
  '';
